...
Create an AWS USER omega-admin and Attach Policy > Administrator Access
  - Create Access_key and Secret_key
```

## Create an AWS Role Called omega-ec2-role and attach it to Master machine
  - Create Role:
    AWS IAM > roles > Create role > AWS Service > Use case (ec2) > Next > AdministratorAccess> Role name (omega-ec2-role) > Create Role
  - Add Role to EC2 Master machine:
    Master machine > Actions > Security > Modify IAM Role > Select omega-ec2-role > Update IAM Role
...
### Install **kubectl** and **eksctl** (Master machine)
  ...
  ```bash
  eksctl create cluster --name=omega \
                     --region=us-west-2 \
                     --version=1.30 \
                     --without-nodegroup
  ```
  - <b> Check clusters
    ```bash
      ...
      Go to AWS CloudFormation, you should see ***eksctl-omega-cluster***
    ```
...
```bash
  ...
  ```bash
  eksctl create nodegroup --cluster=omega \
                       --region=us-west-2 \
                       --name=omega \
                       --node-type=t2.large \
                       --nodes=2 \
...
- <b>Install and Configure ArgoCD (Master Machine)</b>
  ...
  - <b> Change argocd server's service from ClusterIP to NodePort (i.e. port 80:30169 argocd-server) it on security groups of a worker node (omega-omega-Node not omega node)</b>
...
```bash
  ...
  Add cluster information to inform ArgoCD where to deploy:
    $ argocd cluster add cluster-name-from-context --name any-name
      Example: argocd cluster add omega-project-user@omega.us-west-2.eksctl.io --name omega-ekscluster
            omega-project-user@omega.us-west-2.eksctl.io is the concept of RBAC

```
## Create an app in Argocd (browser)
```bash
  Applications > New app:
  - Set these: Application Name: omega | Project Name: default | Sync Policy: Automatic
  - Check mark: Enable Auto-Sync, Prune Resources, Self Heal, Auto-Create Namespace
  - SOURCE: Resource URL: omega project GitHub URL, Revision: main (gitHub branch)
...
```bash
Repo URL: https://github.com/mdjahidhasan6544-ctrl/omega-jenkins-shared-library
...
```bash
eksctl delete cluster --name=omega --region=us-west-2
